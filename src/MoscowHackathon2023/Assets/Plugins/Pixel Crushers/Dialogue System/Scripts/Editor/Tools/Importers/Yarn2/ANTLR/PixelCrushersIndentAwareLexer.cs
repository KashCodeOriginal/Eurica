...
// NOTE: Copied from Yarn's github repo (IndentAwareLexer.cs): https://github.com/YarnSpinnerTool/YarnSpinner/blob/main/YarnSpinner.Compiler/IndentAwareLexer.cs
//       The reason it was copied out is to remove all possibly dependencies on the Yarn codebase from the compilation phase.
//       If changes to the importer are made in the future to support new Yarn syntax, this file may need to be updated to the latest version of Yarn's IndexAwareLexer.cs
namespace PixelCrushers.DialogueSystem.Yarn
{
    ...
    /// </summary>
    public abstract class PixelCrushersIndentAwareLexer : Lexer
    {
        ...
        /// errors to.</param>
        public PixelCrushersIndentAwareLexer(ICharStream input, TextWriter output, TextWriter errorOutput)
        ...
        /// </summary>
        /// <param name="input">The incoming character stream.</param>
        protected PixelCrushersIndentAwareLexer(ICharStream input)
        : base(input)
        {
        }
        ...
        private void CheckNextToken()
        {
            ...
            {
                case PixelCrushersYarnSpinnerLexer.NEWLINE:
                ...
                case PixelCrushersYarnSpinnerLexer.SHORTCUT_ARROW:
                    this.pendingTokens.Enqueue(currentToken);
                    lineContainsShortcut = true;
                    break;

                case PixelCrushersYarnSpinnerLexer.BODY_END:
        ...
        private void HandleEndOfFileToken(IToken currentToken)
        {
            ...
            while (this.unbalancedIndents.Count > 0)
            {
                ...
                // this.InsertToken($"<dedent: {indent}>", PixelCrushersYarnSpinnerLexer.DEDENT);
                this.InsertToken(string.Empty, PixelCrushersYarnSpinnerLexer.DEDENT);
            }
        ...
        private void HandleNewLineToken(IToken currentToken)
        {
            ...
            if (lastSeenOptionContent != -1)
            {
                ...
                if (currentToken.Type == lastToken?.Type)
                {
                    ...
                    if (this.Line - lastSeenOptionContent == 1)
                    {
                        ...
                        // I dislike this and need to look into if you can set a debug text setting in ANTLR
                        // TODO: see above comment
                        // this.InsertToken("<ending option group>", PixelCrushersYarnSpinnerLexer.BLANK_LINE_FOLLOWING_OPTION);
                        this.InsertToken(string.Empty, PixelCrushersYarnSpinnerLexer.BLANK_LINE_FOLLOWING_OPTION);
                    }
            ...
            if (lineContainsShortcut)
            {
                ...
                if (currentIndentationLength > lastIndent)
                {
                    ...
                    // I dislike this and need to look into if you can set a debug text setting in ANTLR
                    // TODO: see above comment
                    // this.InsertToken($"<indent to {currentIndentationLength}>", PixelCrushersYarnSpinnerLexer.INDENT);
                    this.InsertToken(String.Empty, PixelCrushersYarnSpinnerLexer.INDENT);
                }
            ...
            if (unbalancedIndents.Count > 0)
            {
                ...
                while (currentIndentationLength < top)
                {
                    ...
                    // I dislike this and need to look into if you can set a debug text setting in ANTLR
                    // TODO: see above comment
                    // this.InsertToken($"<dedent from {top}>", PixelCrushersYarnSpinnerLexer.DEDENT);
                    this.InsertToken(string.Empty, PixelCrushersYarnSpinnerLexer.DEDENT);
        ...
        {
            if (currentToken.Type != PixelCrushersYarnSpinnerLexer.NEWLINE)
            {
                throw new ArgumentException($"{nameof(this.GetLengthOfNewlineToken)} expected {nameof(currentToken)} to be a {nameof(PixelCrushersYarnSpinnerLexer.NEWLINE)} ({PixelCrushersYarnSpinnerLexer.NEWLINE}), not {currentToken.Type}");
            }
        ...
        private void InsertToken(string text, int type)
        {
            ...
            this.InsertToken(startIndex, startIndex - 1, text, type, this.Line, this.Column, PixelCrushersYarnSpinnerLexer.DefaultTokenChannel);
        }
